...
function calculateOptimumDestination(origin1, origin2){
  ...
  // .then((responses) => {
  //   console.log('back in calc');
  //   console.log(halfpoint2)
  // }

  return {lat: precisionRound(origin1.lat()*percent1,6) + precisionRound(origin2.lat()*percent2,6), lng: precisionRound(origin1.lng()*percent1,6) + precisionRound(origin2.lng()*percent2,6)}
}
